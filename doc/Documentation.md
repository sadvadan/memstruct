...
- Under which scenarios it's advisable to use `unsafe` blocks (aka `#define NMSTRCT 1`) or escape hatches (`m(foo)[i]`)?
    ...
    it's nearly never necessary. memstruct is `C` native - so syscalls or `C` libs that have decades old safety records - can be used as is: suppose, a `C` lib's API has inputs - addr & size - that can be fed as: `M(foo)->addr` and `M(foo)->size` to enforce safety at the interface. or, if you are making a `C` library yourself, you may instead use `foo.id` for greater efficiency - but that's that. in other cases such as say `unsafe` is proven faster in a gainful way (!) in a hot path - can justify `unsafe`.
