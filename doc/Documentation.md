...
- **Memory sharing:** `M(foo)` (a `mstrct *` type pointer to metadata) is simply passed around to share memory (witin or across TUs). many C APIs will implicitly convert this to `void *`. 
```
