...
- **MCU implementation**: a) define `MSTRCT_MCU`. b) define `MSTRCT_PRINT()` and `MSTRCT_ALLOC(bytes, i)` macros in your code; leverage `i` in `MSTRCT_ALLOC` to define custom memory areas for each metadata block, if needed. c) refer `mstrct.h` to match API of these macros and functions. d) make use of memstruct's locks/atomics free multi-threading as discussed below.
