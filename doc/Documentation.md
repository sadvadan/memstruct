...
- **Working theory**: a memory array `foo[ i ][ j ][ k ]..` is constructed of two components-
    ...
    to make use of initializer list in static and on-stack arrays, first declare a memstruct with fixed dimensions e.g. `M(int*, foo,,4)` then `M(storage_keyword, foo, (1,2,5,7))` where `{1,2,5,7}` becomes the initializer list entering the prior fixed dimension in memstruct. 
