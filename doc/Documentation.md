...
- How to allocate memory with spatial checks enabled but temporal checks disabled?
    ...
    **NOTE**: safety suppression is deliberate and best left to user decretion, but e.g. in the arena example it is advisable to make a custom sub-array dummy de-allocator to avoid unnecessary unsafe sections. remember, allocators and de-allocators are drop-in in memstruct.
