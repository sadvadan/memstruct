...
- **Memory sharing:** `M(foo)` (a `void *`) is simply passed around to share memory. 
```
    M(M(foo), bar); // makes bar safely refer the same memory as foo, but retain its type "view"
    callee_function(void* M(foo), other_args); // callee is given M(foo) to access memory and metadata
```
- **Safe access of data:** 
...
- **Re-assign** memory:
    ...
    ```
- **Metadata** access: `M(foo)` is is a `void *` that if cast as `mstrct *` can access `struct {void *addr, uint64_t size}`.
     ```
...
```
    ...
    field access: ((mstrct *)M(foo))->addr, ((mstrct *)M(foo))->size
