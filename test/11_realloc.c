...
int main(void) {
  M(int *,foo,); // declare safe type int[][1]
  M(int *,bar,); // declare safe type int[][1]

  M(malloc(32), foo, 8);
  M(mmap(NULL, 48, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0), bar, 12);

  m(foo,5) = 10; // define int[5][0]
  m(bar,9) = 10; // define int[5][0]
  ...
  M(realloc(M(foo)->addr, 60), foo, 15);
  M(mremap(M(bar)->addr, 48, 44, MREMAP_MAYMOVE), bar, 12);
